...
def isvillage(s, vill, dict, district):
    ...
    if not isvalid(s):
        return False
    #violate = ["atm", "block", "ps", "police", "road", "near", "office", "post", "shop", "bank", "staff", "cell", "market", "hospital",
...
f = open("pres2_shrug_test.csv", "a", encoding = "utf-8")
for enum, x in enumerate(pres_addresses):
    id = fir_id[enum]
    perm = x
    if type(perm) == str and enum > 173534:
