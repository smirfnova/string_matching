...
def isvillage(s, vill, dict, district):
    s = s.lower()
    vill = vill.lower()
...
def four_elem(elems, perm, dict, f, data, id):
    country = elems[-1]
    state = elems[-2]
    district = elems[-3]
    vill = elems[0]
    ...
    if vill[0:3] == "ps ":
        vill = vill[3:]
    if state == "haryana":
        if isvillage(perm, vill, dict, district):
            #print("isvillage")
            if not "vill" in elems[0].lower() and not "vil" in elems[0].lower() and not "village" in elems[0].lower() and not "vila" in elems[0].lower() and not "vilage" in elems[0].lower() and not "will" in elems[0].lower() and not "vilalge" in elems[0].lower() and not elems[0].lower()[0:4] == "will ":
                vill = vill.lower()
                ...
                if len(vill) > 0:
                    ...
                    if "distt" in vill.lower(): #fix to be correct district
                        district = separate_dis(district, vill, dict, "distt")
                    if "district" in vill.lower():
                        district = separate_dis(district, vill, dict, "district")
                    ...
                    #print(min_str, "  ", min)
                    #print(use)
                    if min != 100 and type(haryana_id) != str:    
                        print(perm, min_str)  
                        write = str(id) + "," + perm.replace(",", "|") + "," + str(haryana_id) + "," + min_str.replace(",", "|") + "\n"
                        f.write(write)
                                    

data = pd.read_csv("shrug_names.csv")
...
f = open("perm2_shrug_test.csv", "a", encoding = "utf-8")
for enum, x in enumerate(perm_addresses):
    id = fir_id[enum]
    perm = x
    if type(perm) == str and enum > 619788:
                            ...
                            if not "vill" in elems[0].lower() and not "vil" in elems[0].lower() and not "village" in elems[0].lower() and not "vila" in elems[0].lower() and not "vilage" in elems[0].lower() and not "will" in elems[0].lower() and not "vilalge" in elems[0].lower() and not elems[0].lower()[0:4] == "will ":
                                ...
                                if len(vill) > 0:
                                    ...
                                    if min != 100 and type(haryana_id) != str:   
                                        ...
                                        #sai_index = data.index[data['gp_code']==gp_code].tolist()[0]
                                        #sai_line = sai_lines[sai_index]
                                        #sai_line = data.loc[data['gp_code'] == gp_code].apply(str).values
                                        #print(gp_code)
                                        
                                        #sai_line = ','.join(sai)
                                        #print(sai_line)

                                        #write = str(id) + "," + str(perm.replace(",", "|")) + "," + str(min_str.replace(",", "|")) + "," + str(haryana_id) + "," + str(gp_code.replace(",", "|")) + "\n"
                                        print(perm, min_str)
                                        write = str(id) + "," + perm.replace(",", "|") + "," + str(haryana_id) + "," + min_str.replace(",", "|") + "\n"
                                        f.write(write)
                if len(elems) == 4:
                    four_elem(elems, perm, dict, f, data,id)
